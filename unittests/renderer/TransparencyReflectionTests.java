...
class TransparencyReflectionTests {
    ...
    @Test
    @SuppressWarnings("java:S109")
    void mySpecialImageTest() {
        // Clear previous configurations to build a custom scene
        _scene.setAmbientLight(new AmbientLight(new Color(20, 20, 20)));

        _scene.geometries.add(
                // 1. Large semi-transparent blue outer sphere
                new Sphere(new Point(0, 0, -50), 60D)
                        .setEmission(new Color(0, 50, 100))
                        .setMaterial(new Material().setKD(0.2).setKS(0.3).setShininess(50).setKT(0.7)),

                // 2. Smaller reflective pink inner sphere
                new Sphere(new Point(0, 0, -50), 30D)
                        .setEmission(new Color(120, 20, 60))
                        .setMaterial(new Material().setKD(0.4).setKS(0.5).setShininess(100).setKR(0.5)),

                // 3. A matte grey triangle acting as a floor underneath (Object 3)
                new Triangle(new Point(-100, -80, -100), new Point(100, -80, -100), new Point(0, -80, 50))
                        .setEmission(new Color(30, 30, 30))
                        .setMaterial(new Material().setKD(0.5).setKS(0.2).setShininess(10)),

                // 4. A mirror triangle standing in the background (Object 4)
                new Triangle(new Point(-150, -50, -150), new Point(-100, 100, -150), new Point(-150, 100, -50))
                        .setEmission(new Color(10, 10, 10))
                        .setMaterial(new Material().setKR(0.8))
        );

        // Light source setup matching your project's architecture
        _scene.lights.add(
                new SpotLight(new Color(600, 300, 300), new Point(60, 50, 100), new Vector(-1, -1, -2))
                        .setKl(0.0004).setKq(0.0000006)
        );

        // Camera build and render pipeline exact to your architecture
        _cameraBuilder
                .setLocation(new Point(0, 0, 1000))
                .setDirection(Point.ZERO, Vector.AXIS_Y)
                .setVpDistance(1000).setVpSize(160, 160)
                .setResolution(500, 500)
                .build()
                .renderImage()
                .writeToImage("mySpecialImage");
    }
}
